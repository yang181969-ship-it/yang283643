 ...
 * Phase 3: 侧边栏折叠切换
 * -----------------------------------------------------------
 * 状态真源:<html data-nav="collapsed|expanded">
 * 持久化:localStorage['navCollapsed'](字符串 'true'/'false')
 *
 * 初始状态由 index.html <head> 里的 inline script 注入(防闪烁),
 * 这里只管:
 *   1. 点击 .nav-toggle 切换 data-nav + 写 localStorage
 *   2. 更新 aria-expanded
 *
 * 不改动 .nav-link 的 active 高亮逻辑 —— 那是上面 initNavAutoCenter
 * 及 SPA 路由的职责,折叠态只影响视觉不影响语义
 * =========================================================== */
(function initNavCollapse() {
  function setup() {
    var toggle = document.querySelector('.rail-toggle, .nav-toggle');
    if (!toggle) return;

    // 同步 aria-expanded 到当前 data-nav 状态
    function syncAria() {
    ...
    toggle.addEventListener('click', function () {
      ...
      var next = !current;
      document.documentElement.setAttribute('data-nav', next ? 'collapsed' : 'expanded');
      try {
        localStorage.setItem('navCollapsed', next ? 'true' : 'false');
      } catch (e) {
        // localStorage 不可用时静默失败,折叠状态仅当前会话有效
      }
      syncAria();
    });
  }
